...
# Try to import PyPDF2 as backup
try:
    import PyPDF2
...
def extract_text_from_upload(file_path: str, file_bytes: bytes) -> str:
    ...
    if ext.endswith(".pdf"):
        ...
        try:
            ...
            if full_text.strip():
                logging.info(f"Successfully extracted {len(full_text)} characters using pdfplumber.")
                return full_text.strip()
            else:
                logging.warning("pdfplumber extracted empty text. Trying PyPDF2...")
        except Exception as e:
            logging.warning(f"pdfplumber failed: {e}. Trying PyPDF2...")
        ...
        if PYPDF2_AVAILABLE:
            try:
                ...
                if full_text.strip():
                    logging.info(f"Successfully extracted {len(full_text)} characters using PyPDF2.")
                    return full_text.strip()
                else:
                    logging.warning("PyPDF2 extracted empty text. Trying Textract...")
            except Exception as e:
                logging.warning(f"PyPDF2 failed: {e}. Falling back to Textract.")
        else:
            logging.warning("PyPDF2 not available. Falling back to Textract.")
    ...
    if textract_client is not None:
        ...
        try:
            ...
            if error_code == 'InvalidParameterException':
                ...
                logging.error("PDF may be password-protected, corrupted, or in an unsupported format")
                logging.error("Try: 1) Remove password protection, 2) Re-save PDF, 3) Convert to image format")
                return ""
            else:
